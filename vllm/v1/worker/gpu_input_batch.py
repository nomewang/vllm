...
class CachedRequestState:
    ...
    # For multi-layer MTP/EAGLE proposer state.
...
class InputBatch:
    ...
    def add_request(
        ...
        if self.multi_layer_eagle_num > 0:
            assert self.cached_len is not None
            assert self.cached_token_ids is not None
            assert self.cached_hidden_states is not None
            assert self.cached_slot_mappings is not None
            assert self.cached_positions is not None
            assert request.cached_len is not None
            assert request.cached_token_ids is not None
            assert request.cached_hidden_states is not None
            assert request.cached_slot_mappings is not None
            assert request.cached_positions is not None
